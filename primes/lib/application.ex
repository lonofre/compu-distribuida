defmodule Primes.Application do
    ...
    def start(_, _) do
        ...
        {:ok, spawn(fn -> 
            case mode do
                "sieve" -> Server.sieve(upper_bound)
                "range" -> Server.range_primes_finder(n_workers, upper_bound) |> write_primes(@file_name)
                "dispenser" -> Server.dispenser_primes_finder(n_workers, upper_bound) |> write_primes(@file_name)
            end
        end)}
    end
